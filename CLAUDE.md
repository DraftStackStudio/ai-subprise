...
AI Subprise is a personal AI account directory that lets users track which email account is linked to each AI tool, along with subscription status, billing type, and trial expiry — all from a clean, searchable dashboard.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "238494489+DraftStackStudio@users.noreply.github.com"
  git config user.name "DraftStackStudio"
  ```
